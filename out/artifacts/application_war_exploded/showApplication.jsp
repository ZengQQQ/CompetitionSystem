                                ...
                                <div class="box"><div class="container push-left d-flex justify-content-center align-items-center" ng-app="winnieApp" ng-controller="winnieCtrl">
                                    <table class="table table-striped">
                                        ...
                                        <tbody style="font-size: 18px">
                                            <c:forEach items="${application_list}" var="application">
                                                <tr ng-repeat="data in information | filter:search">
                                                    <td>${application.getTime()}</td>
                                                    <td>${application.getStudent().getStudent_ID()}</td>
                                                    <td>${application.getStudent().getStudent_Name()}</td>
                                                    <td>${application.getStudent().getTel()}</td>
                                                    <td>${application.getStudent().getMajor()}</td>
                                                    <td>${application.getStudent().getStrong_point()}</td>
                                                    <td>
                                                        <a class="action-link" href="javascript:tip_agree(${application.getStudent().getStudent_ID()})">同意</a>
                                                        <a class="action-link" href="javascript:tip_refuse(${application.getStudent().getStudent_ID()})">拒绝</a>
                                                    </td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
...
<script>
    function tip_agree(id){
        if(confirm("是否同意入队申请？")){
            window.location.href="${pageContext.request.contextPath}/agree_Application?id="+id;
        }else {
            return false;
        }
    }

    function tip_refuse(id){
        if(confirm("是否拒绝入队申请？")){
            window.location.href="${pageContext.request.contextPath}/refuse_Application?id="+id;
        }else {
            return false;
        }
    }
